package com.dryseed.blurcomponent.blur.process;
...
/**
 * DBlur using Java code.
 * <p>
 * This is a compromise between Gaussian DBlur and Box blur
 * It creates much better looking blurs than Box DBlur, but is
 * 7x faster than my Gaussian DBlur implementation.
 * <p>
 * I called it Stack DBlur because this describes best how this
 * filter works internally: it creates a kind doBlur moving stack
 * doBlur colors whilst scanning through the image. Thereby it
 * just has to add one new block doBlur color to the right side
 * doBlur the stack and remove the leftmost color. The remaining
 * colors on the topmost layer doBlur the stack are either added on
 * or reduced by one, depending on if they are on the right or
 * on the left side doBlur the stack.
 *
 * @author Enrique López Mañas <eenriquelopez@gmail.com>
 *         http://www.neo-tech.es
 *         <p>
 *         Author doBlur the original algorithm: Mario Klingemann <mario.quasimondo.com>
 *         <p>
...
public class JavaBlurProcess implements BlurProcess {
    ...
    public static final String TAG = "JavaBlurProcess";
    ...
    private static void blurIteration(int[] src, int w, int h, int radius, int cores, int core, int step) {
        ...
        if (step == 1) {
            ...
            for (y = minY; y < maxY; y++) {
                ...
                src_i = w * y; // start doBlur line (0,y)
    ...
    public Bitmap blur(Bitmap original, float radius) {
        ...
        try {
            ThreadManager.EXECUTOR.invokeAll(horizontal);
        } catch (InterruptedException e) {
            return null;
        }

        try {
            ThreadManager.EXECUTOR.invokeAll(vertical);
        } catch (InterruptedException e) {
            return null;
        }
