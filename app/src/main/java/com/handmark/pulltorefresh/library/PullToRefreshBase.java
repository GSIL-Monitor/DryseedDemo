...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public final static float OFFSET_RADIO = FRICTION;
	...
	public void addHeaderView(View header, ViewGroup.LayoutParams params) {
		mHeaderLayout.addHeaderView(header, params);
	}

	public void addFooterView(View header, ViewGroup.LayoutParams params) {
		mFooterLayout.addHeaderView(header, params);
	}
	...
	protected void onScrollChanged(int l, int t, int oldl, int oldt) {
		...
		mHeaderLayout.onScroll(l, -t);
		mFooterLayout.onScroll(l, -t);
	}
	...
	/**
	 * @deprecated You should now call this method on the result of {@link #getBaseLoadingLayoutProxy()}.
	 */
	...
	/**
	 * @deprecated You should now call this method on the result of {@link #getBaseLoadingLayoutProxy(boolean, boolean)}.
	 */
	...
	protected BaseLoadingLayout createLoadingLayout(Context context, Mode mode, TypedArray attrs) {
		BaseLoadingLayout layout = mLoadingAnimationStyle.createLoadingLayout(context, mode, getPullToRefreshScrollDirection(), attrs);
		layout.setVisibility(View.INVISIBLE);
		return layout;
	}

	/**
	 * Used internally for {@link #getBaseLoadingLayoutProxy(boolean, boolean)}. Allows derivative classes to include any extra BaseLoadingLayouts.
	 */
	...
	/**
	 * This is implemented by derived classes to return the created View. If you need to use a custom View (such as a custom ListView), override this method and return an instance of your custom
	 * class.
	 * <p/>
	...
	/**
	 * Called by {@link #onRestoreInstanceState(Parcelable)} so that derivative classes can handle their saved instance state.
	 * 
	...
	protected final void setHeaderScroll(int value) {
		...
		if (USE_HW_LAYERS) {
			/**
			 * Use a Hardware Layer on the Refreshable View if we've scrolled at all. We don't use them on the Header/Footer Views as they change often, which would negate any HW layer performance
			 * boost.
			 */
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (a.hasValue(R.styleable.PullToRefresh_ptrMode)) {
            ...
            String modeString = a.getString(R.styleable.PullToRefresh_ptrMode);
            mMode = Mode.mapStringToValue(modeString);
        }
        ...
        if (a.hasValue(R.styleable.PullToRefresh_ptrAnimationStyle)) {
            ...
            // TODO: 由于组件aapt的限制, 以上的方法, a.getInteger不能正常返回属性值对应的数字序号. 所以会崩溃.
            // TODO: 目前先用以下方法来绕过此项限制. 等aapt完善后再改回来.
            String animationTypeString = a.getString(R.styleable.PullToRefresh_ptrAnimationStyle);
			mLoadingAnimationStyle = AnimationStyle.mapStringToValue(animationTypeString);
		}
	...
	public static enum AnimationStyle {
		/**
		 * This is the default for Android-PullToRefresh. Allows you to use any drawable, which is automatically rotated and used as a Progress Bar.
		 */
		ROTATE,

		/**
		 * This is the old default, and what is commonly used on iOS. Uses an arrow image which flips depending on where the user has scrolled.
		 */
		FLIP,
		...
		/**
		 * Maps an int to a specific mode. This is needed when saving state, or inflating the view from XML where the mode is given through a attr int.
		 * 
		 * @param modeInt
        ...
        static AnimationStyle mapStringToValue(String strMode) {
            if (strMode == null || strMode.isEmpty()) {
                return JINGDONG;
            }
            if (strMode.toLowerCase().equals("rotate")) {
                return ROTATE;
            } else if (strMode.toLowerCase().equals("flip")) {
                return FLIP;
            } else if (strMode.toLowerCase().equals("jingdong")) {
                return JINGDONG;
            } else if (strMode.toLowerCase().equals("jingdong_flip")) {
		...
		BaseLoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs) {
			switch (this) {
				case JINGDONG:
				default:
					return new JDLoadingLayout(context, mode, scrollDirection, attrs);
				case FLIP:
	...
	public static enum Mode {
		...
		/**
		 * Only allow the user to Pull from the start of the Refreshable View to refresh. The start is either the Top or Left, depending on the scrolling direction.
		 */
		PULL_FROM_START(0x1),

		/**
		 * Only allow the user to Pull from the end of the Refreshable View to refresh. The start is either the Bottom or Right, depending on the scrolling direction.
		 */
		PULL_FROM_END(0x2),
		...
		/**
		 * Disables Pull-to-Refresh gesture handling, but allows manually setting the Refresh state via {@link PullToRefreshBase#setRefreshing() setRefreshing()}.
		 */
		 ...
		 */
		public final static Mode PULL_DOWN_TO_REFRESH = Mode.PULL_FROM_START;

		/**
		 * @deprecated Use {@link #PULL_FROM_END} from now on.
		 */
		public final static Mode PULL_UP_TO_REFRESH = Mode.PULL_FROM_END;
        ...
        static Mode mapStringToValue(String strMode) {
            if (strMode == null || strMode.isEmpty()) {
                return DISABLED;
            }
            if (strMode.toLowerCase().equals("disabled")) {
                return DISABLED;
            } else if (strMode.toLowerCase().equals("pullFromStart")) {
                return PULL_FROM_START;
            } else if (strMode.toLowerCase().equals("pullFromEnd")) {
                return PULL_FROM_END;
            } else if (strMode.toLowerCase().equals("both")) {
                return BOTH;
            } else if (strMode.toLowerCase().equals("manualOnly")) {
	...
	/**
	 * Simple Listener that allows you to be notified when the user has scrolled to the end of the AdapterView. See ( {@link PullToRefreshAdapterViewBase#setOnLastItemVisibleListener}.
	 * 
	 * @author Chris Banes
	 */
	...
	/**
	 * Listener that allows you to be notified when the user has started or finished a touch event. Useful when you want to append extra UI events (such as sounds). See (
	...
	public static interface OnPullEventListener<V extends View> {
		 ...
		 * @param direction
		 *            - One of {@link Mode#PULL_FROM_START} or {@link Mode#PULL_FROM_END} depending on which direction the user is pulling. Only useful when <var>state</var> is
	...
	/**
	 * An advanced version of the Listener to listen for callbacks to Refresh. This listener is different as it allows you to differentiate between Pull Ups, and Pull Downs.
	 * 
	 * @author Chris Banes
	 */
	...
	public static enum State {
		...
		/**
		 * When the UI is being pulled by the user, but has not been pulled far enough so that it refreshes when released.
		 */
		PULL_TO_REFRESH(0x1),

		/**
		 * When the UI is being pulled by the user, and <strong>has</strong> been pulled far enough so that it will refresh when released.
		 */
		...
		/**
		 * When the UI is currently refreshing, caused by a call to {@link PullToRefreshBase#setRefreshing() setRefreshing()}.
		 */
