...
public class JobConsumerExecutor {
    ...
    public JobConsumerExecutor(Configuration config, Contract contract) {
        ...
        LogUtil.d("maxConsumer:" + maxConsumerSize + " " + "  minConsumer:" + minConsumerSize);
    }
            ...
            boolean res =
                    consumerCnt < minConsumerSize ||
                            consumerCnt * loadFactor < contract.countRemainingReadyJobs() +
                                    runningJobHolders.size();
            if (JqLog.isDebugEnabled()) {
                JqLog.d("%s: load factor check. %s = (%d < %d)|| (%d * %d < %d + %d). consumer " +
                                "thread: %s",
                        Thread.currentThread().getName(),
                        res,
                        consumerCnt, minConsumerSize,
                        consumerCnt, loadFactor, contract.countRemainingReadyJobs(),
                        runningJobHolders.size(), inConsumerThread);
            }
            return res;
        }
    ...
    public static interface Contract {
        ...
        /**
         * should insert the given {@link JobHolder} to related {@link JobQueue}. if it already
         * exists, should replace the
         * existing one.
         *
         * @param jobHolder
         */
            ...
            do {
                ...
                try {
                    ...
                    do {
                        ...
                        nextJob = null;
                        job = null;
                        nextJob = contract.isRunning() ? contract.getNextJob(executor
                                .keepAliveSeconds, TimeUnit.SECONDS) : null;
                        if (nextJob != null) {
                            ...
                            JobMonitor.onBeforeRun(nextJob);

                            job.run();

                            JobMonitor.onAfterRun(nextJob);

                            try {
                                if (nextJob != null && nextJob.baseJob != null) {
                                    threadName = nextJob.getBaseJob().getJobName();
                                    if (!TextUtils.isEmpty(threadName)) {
                                        LogUtil.d(JobManagerUtils.TAG, "current working thread " +
                                                "name:" + threadName);
                                        Thread.currentThread().setName("TM_" + threadName);
                                    }
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
                    ...
                    if (JqLog.isDebugEnabled()) {
                        if (canDie) {
                            LogUtil.d("finishing consumer:" + Thread.currentThread().getName());
                            recycledRunnableCount.decrementAndGet();

                        } else {
                            String previousThreadName = Thread.currentThread().getName();

                            if (!previousThreadName.startsWith("TM#")) {
                                Thread.currentThread().setName("TM#" + recycledRunnableCount
                                        .getAndIncrement());
                                LogUtil.d("didn't allow me to " +
                                        "die," +
                                        " re-running:" + previousThreadName +
                                        " transform to :" + Thread.currentThread().getName());
                            }
                        }
                        LogUtil.d("jobmanager thread:" + activeConsumerCount.get());
                    }
                }
            } while (!canDie);
        }
    ...
    private static class FutureJob extends FutureTask {
        ...
        public FutureJob(@NonNull Contract contract, @NonNull JobHolder jobHolder, @NonNull
                Callable callable) {
            super(callable);
