...
public class Caton {
    ...
    UILooperObserver mUiLooperObserver;
    ...
    private Caton(Context context, long thresholdTimeMillis, long collectIntervalMillis, MonitorMode mode, boolean loggingEnabled, Callback callback) {
        ...
        mBlockHandler = new BlockHandler(context, mTraceCollector, callback);
        if (mode == MonitorMode.LOOPER) {
            mUiLooperObserver = new UILooperObserver(mBlockHandler);
    ...
    private void stop(){
        if(null != mUiLooperObserver){
            mUiLooperObserver.stop();
        }
        mBlockHandler = null;
    }
