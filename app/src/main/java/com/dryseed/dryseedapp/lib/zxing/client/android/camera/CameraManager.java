...
package com.dryseed.dryseedapp.lib.zxing.client.android.camera;
...
import com.dryseed.dryseedapp.lib.zxing.client.android.PlanarYUVLuminanceSource;
 ...
 * 
 */
@SuppressLint("NewApi") public final class CameraManager {

	private static final String TAG = CameraManager.class.getSimpleName();

	private static final int MIN_FRAME_WIDTH = 240;// 最低分辨率宽度为240
	private static final int MIN_FRAME_HEIGHT = 240;
	private static final int MAX_FRAME_WIDTH = 1080;// 增加适配到1920x1080
	 ...
	 *             Indicates the camera driver failed to open.
	 */
	public void openDriver(Activity activity,SurfaceHolder holder,int index,int cameraPosition) throws Exception {
		if (camera == null) {
			try {
...
//				configManager.setDisplayOrientation(camera,270);
				int degree = getCameraDegrees(activity,index,camera);
				//((CaptureActivity)activity).setReverseDegree(degree);
	...
	public static int getCameraDegrees (Activity activity, int cameraId, Camera camera) {
	...
	/**
	 * 修复黑屏问题 Closes the camera driver if still in use.
	 */
	...
	/**
	 * Switch on or switch off the flash light
	 * 
	 * @param isOn
	 *            a boolean value indicates on or off
	 */
	public void switchFlashlight(boolean isOn) {
		if (camera == null) {
			return;
		}
		if (SDK_INT >= 5) {
			Parameters parameter = camera.getParameters();  
			parameter.setFlashMode(isOn ? Parameters.FLASH_MODE_TORCH : Parameters.FLASH_MODE_OFF);
	...
	public void setZoom(int value) {
		...
		if (currentZoomValue < 0) {
			currentZoomValue = 0;
		} else if (currentZoomValue > maxZoomValue) {
			currentZoomValue = maxZoomValue;
		}
//		com.jingdong.common.utils.Log.e("ZY", "value: " + value + ", currentZoomValue: " + currentZoomValue + ", maxZoomValue: " + maxZoomValue);

		if (SDK_INT >= 8) {  
			parameter.setZoom(currentZoomValue);
			camera.setParameters(parameter);
		}
	}
