package com.dryseed.dryseedapp.customView.foldingLayout.view;
...
public class FoldLayout extends ViewGroup
{
	...
	private void updateFold()
	{
		...
		mTranslateDis = w * mFactor; // mFactor = Math.abs(((float) mTranslation) / ((float) getWidth()))
	...
	protected void dispatchDraw(Canvas canvas)
	{
		...
		for (int i = 0; i < mNumOfFolds; i++)
		{
			...
			canvas.clipRect(mFlodWidth * i, 0, mFlodWidth * i + mFlodWidth,
					getHeight());
			/*
				在我们自己new的mBitmap中也绘制了一份图片，因为我不希望每次都是调用super.dispatchDraw，
				所以只要isReady=true，我们就可以去调用绘制mBitmap而避免调用super.dispatchDraw()。
			 */
			if (isReady)
			{
