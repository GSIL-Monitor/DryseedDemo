...
public class BinarySlidingMenu extends HorizontalScrollView {
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
		...
		/*
		1、scale，在滑动左侧菜单时：值为1.0~0.0；mMenuWidth * (scale - 1) 的值就是从 0.0~ -mMenuWidth（注意：负的） ；
		那么mContent的向左偏移量，就是0到mMenuWidth ；也就是说，整个滑动的过程，我们强制让内容区域固定了。
		2、scale，在滑动右侧菜单时：值为：1.0~2.0；mMenuWidth * (scale - 1) 的值就是从 0.0~ mMenuWidth（注意：正数） ；
		那么mContent的向右偏移量，就是0到mMenuWidth ；也就是说，整个滑动的过程，我们强制让内容区域固定了。
		 */
        float scale = l * 1.0f / mMenuWidth;
        mContent.animate().translationX(mMenuWidth * (scale - 1));
    }
}
