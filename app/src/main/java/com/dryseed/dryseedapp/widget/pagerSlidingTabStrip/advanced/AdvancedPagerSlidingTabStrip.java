...
package com.dryseed.dryseedapp.widget.pagerSlidingTabStrip.advanced;
...
public class AdvancedPagerSlidingTabStrip extends HorizontalScrollView {
     ...
     * 默认的指示器颜色
     */
    protected static final int DEFAULT_INDICATOR_COLOR = 0xff0bbe06;
     ...
     * 标签文字颜色
     */
    protected ColorStateList mTabTextColor = ContextCompat.getColorStateList(getContext(), R.color.pager_sliding_tab_color);
    /**
     * Tab的默认背景颜色
     */
     ...
     * 指示器距离底边padding
     */
    protected int mIndicatorBottomPadding = 0;
    /**
     * 下划线高度
     * 现在用dp表示 会在构造函数时转换为px
     */
    ...
    private SparseIntArray mSpecialTabPaddingLeftArray = new SparseIntArray();
    private SparseIntArray mSpecialTabPaddingRightArray = new SparseIntArray();
    /**
     * 文字大小
    ...
    protected void updateInGlobalLayoutListener() {
        ...
        if (mTabsContainer.getChildCount() > mCurrentPosition) {
            ...
            if (mIsCustomTab && tab instanceof ViewGroup) {
                setCustomTabSelected((ViewGroup) tab, true);
            } else if (tab instanceof Checkable) {
    ...
    protected void setCustomTabSelected(ViewGroup parent, boolean selected) {
        if (parent == null) {
            return;
        }
    ...
    protected void clearAllCustomTabSelect() {
        int size = mTabsContainer.getChildCount();
        for (int i = 0; i < size; i++) {
            View tab = mTabsContainer.getChildAt(i);
            if (tab instanceof ViewGroup) {
                setCustomTabSelected((ViewGroup) tab, false);
            }
        }
    }
    ...
    protected void addTab(final int position, View tab) {
        ...
        if (mShouldExpand) {
            ...
            setTabPaddingFirstTime(tab, position);
        }
    ...
    protected RadioGroup.LayoutParams getExpandedTabLayoutParams() {
        return new RadioGroup.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT, 1.0f);
    }
    ...
    public View scrollToChild(int position, int offset) {
        ...
        if (newScrollX != mLastScrollX) {
            ...
            if (mToCenter) {
                ...
                smoothScrollTo(left + childWith / 2 - mScreenWidth / 2 + getLeft(), 0);
                return child;
            } else {
     ...
     */
    protected void updateTextViewTabStyle(TextView tab, int position) {
     ...
     * @param position tab位置
     */
    private void setTabPaddingFirstTime(View tab, int position) {
        int leftPadding = mSpecialTabPaddingLeftArray.get(position, mTabPadding);
        int rightPadding = mSpecialTabPaddingRightArray.get(position, mTabPadding);
        tab.setPadding(leftPadding, 0, rightPadding, 0);
    }
     ...
     * @param position tab位置
     */
    private void setTabPaddingInternal(View tab, int position) {
        int tabLeftPadding = tab.getPaddingLeft();
        int tabRightPadding = tab.getPaddingRight();
        int tabTopPadding = tab.getPaddingTop();
        int tabBottomPadding = tab.getPaddingBottom();
        int leftPadding = mSpecialTabPaddingLeftArray.get(position, 0);
        int rightPadding = mSpecialTabPaddingRightArray.get(position, 0);
        leftPadding = leftPadding != 0 ? leftPadding : tabLeftPadding;
        rightPadding = rightPadding != 0 ? rightPadding : tabRightPadding;
        if (leftPadding != tabLeftPadding || rightPadding != tabRightPadding) {
            tab.setPadding(leftPadding, tabTopPadding, rightPadding, tabBottomPadding);
        }
    }
    ...
    private void setTabColorGradient() {
        View currentTab = mTabsContainer.getChildAt(mCurrentPosition);
        if (FloatUtil.equals(mCurrentPositionOffset, 0)) {
            if (mIsCustomTab) {
                clearAllCustomTabSelect();
            }
            if (mIsCustomTab && currentTab instanceof ViewGroup) {
                setCustomTabSelected((ViewGroup) currentTab, true);
            } else if (currentTab instanceof Checkable) {
    ...
    protected void drawIndicatorLine(Canvas canvas, int height) {
        ...
        View currentTab = mTabsContainer.getChildAt(mCurrentPosition);
        float center = (currentTab.getLeft() + currentTab.getRight() +
                currentTab.getPaddingLeft() - currentTab.getPaddingRight()) / 2f;
        if (center <= 0) {
            return;
        }
    ...
    public void setCustomTabProvider(ICustomTabProvider customTabProvider) {
    ...
    public int getIndicatorBottomPadding() {
        return mIndicatorBottomPadding;
    }

    public void setIndicatorBottomPadding(int indicatorBottomPadding) {
        if (this.mIndicatorBottomPadding != indicatorBottomPadding) {
            this.mIndicatorBottomPadding = indicatorBottomPadding;
            invalidate();
        }
    }
    ...
    public void setSpecialTabPaddingLeft(int position, int paddingPx, boolean updateNow) {
        mSpecialTabPaddingLeftArray.put(position, paddingPx);
        if (updateNow) {
            updateTabStyles();
        }
    }

    public void setSpecialTabPaddingRight(int position, int paddingPx, boolean updateNow) {
        mSpecialTabPaddingRightArray.put(position, paddingPx);
        if (updateNow) {
            updateTabStyles();
        }
    }
    ...
    protected void setBoldTypeface(int position) {
