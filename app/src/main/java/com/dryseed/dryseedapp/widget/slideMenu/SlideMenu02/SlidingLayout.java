package com.dryseed.dryseedapp.widget.slideMenu.SlideMenu02;
...
public class SlidingLayout extends RelativeLayout implements OnTouchListener {
	 ...
	 *
	 * 然后我们看一下setScrollEvent方法，这个方法接收一个View作为参数，然后为这个View绑定了一个touch事件。这是什么意思呢？
	 * 让我们来想象一个场景，如果右侧布局是一个LinearLayout，我可以通过监听LinearLayout上的touch事件来控制左侧布局的显示和隐藏。
	 * 但是如果右侧布局的LinearLayout里面加入了一个ListView，而这个ListView又充满了整个LinearLayout，这个时候LinearLayout将不可能再被touch到了，这个时候我们就需要将touch事件注册到ListView上。
