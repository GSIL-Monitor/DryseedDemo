...
public class ProcessUtils {
    ...
    public static void waitForOrKill(Process command, long numberOfMillis) {
        ProcessRunner runnable = new ProcessRunner(command);
    ...
    public static Thread consumeProcessErrorStream(Process command, OutputStream err) {
        Thread thread = new Thread(new ByteDumper(command.getErrorStream(), err));
        thread.start();
        return thread;
    }

    public static Thread consumeProcessErrorStream(Process command, Appendable error) {
        Thread thread = new Thread(new TextDumper(command.getErrorStream(), error));
        thread.start();
        return thread;
    }

    public static Thread consumeProcessOutputStream(Process command, Appendable output) {
        Thread thread = new Thread(new TextDumper(command.getInputStream(), output));
        thread.start();
        return thread;
    }

    public static Thread consumeProcessOutputStream(Process command, OutputStream output) {
        Thread thread = new Thread(new ByteDumper(command.getInputStream(), output));
        thread.start();
        return thread;
    }
